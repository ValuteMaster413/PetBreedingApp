        ...
        <div className="profile-container">
            ...
            {isEditingPet && petToEdit && (
                <Modal onClose={() => {
                    ...
                    <PetEditor
                        pet={petToEdit}
                        onSave={async (updatedPet, newPhotos) => {
                            try {
                                const formData = new FormData();
                                ...
                                const csrf = await getCsrfToken();
                                const res = await axios.post(
                                    `http://localhost:8000/pets/edit_pet/${updatedPet.id}/`,
                                    formData,
                                    {
                                        headers: {
                                            "X-CSRFToken": csrf,
                        ...
                        onDeletePhotos={async (photoIds) => {
                            try {
                                const formData = new FormData();
                                photoIds.forEach(id => formData.append("delete_photo", id));
